...
### Cozum: birim fiyati gunluk getiriden geriye zincirleme

Gunluk getiri yuzdesi bes yillik mevcut oldugundan, bugunun bilinen fiyatindan
geriye dogru zincirleme yapilabilir:

```
nav(t-1) = nav(t) / (1 + daily_return_pct(t) / 100)
```

Yontem TLY fonunda dogrulandi: 2026-08-31 icin kayitli fiyat 9145.580810,
zincirle turetilen 9145.580850 — sekiz hane tutuyor.

Bu tek seferlik bir geriye doldurma isidir ve grafigin on kosuludur. Turetilen
satirlar, olculen satirlardan ayirt edilebilir kalmalidir; `source` alani bu
ayrimi tasir ve olculen bir deger geldiginde turetilen deger onun yerine gecmez.
...
- Birim fiyat gecmisi geriye doldurulmustur: son 30 is gunu icin portfoydeki
  her fonun `nav_per_share` degeri doludur.
- Turetilen fiyat satirlari `source` alaninda olculen satirlardan ayirt edilir;
  kaynaktan olculmus bir deger geldiginde turetilen deger onun uzerine yazmaz.
- Zincirleme dogrulugu test edilir: bilinen bir fiyata sahip gun, kendisinden
  sonraki gunun fiyati ve getirisinden geriye zincirlendiginde ayni degeri
  makul bir tolerans icinde verir.
